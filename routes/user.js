...
router.post("/love", async function (req, res) {
    ...
    try {
        ...
        var otherUserRequest = await models.LoveRequest.findOne({ userId: userId})
        ...
        if(existingRequest){
            ...
            var love = await models.Love.findOne({ userId: userIdToLove })
                        .select("loveId type");

            if(love){
                love = love.toJSON();

                var userLove = await models.User.findOne({id: love.loveId})
                        .select("id name avatar -_id");

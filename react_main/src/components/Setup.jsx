...
export default function Setup(props) {
  ...
  const disablePopover = props.disablePopover;
	...
	function selectSetup(index) {
		let roleNames = sortRoles(Object.keys(props.setup.roles[index]), props.setup.gameType, siteInfo);
...
function sortRoles(roleList, gameType, siteInfo) {
	...
	listCopy.sort((a, b) => {
		...
		let modA = modifiersA?.split("_").filter(e => e).map(e => siteInfo.modifiers[gameType].find(x => x.name === e)).find(e => e.alignment);
		let modB = modifiersB?.split("_").filter(e => e).map(e => siteInfo.modifiers[gameType].find(x => x.name === e)).find(e => e.alignment);
...
export function SmallRoleList(props) {
	...
	if (Array.isArray(props.roles)) {
		...
		roles = sortRoles(Object.keys(props.roles), props.gameType, siteInfo).map(role => (
			<RoleCount
				role={role}

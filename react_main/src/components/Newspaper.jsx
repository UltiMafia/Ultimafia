...
function ObituaryItem({ death, onFullyAnimated, parentProps }) {
  ...
  const revealResult = isAlignmentReveal
    ? getAlignmentName(death.revealMessage)
    : reformatRoleName(getRoleName(death.revealMessage));
  ...
  const [animatedDeathMessage, setAnimatedDeathMessage] = useState(
    noAnimation ? true : false
  );
  const [animatedRevealMessage, setAnimatedRevealMessage] = useState(
    noAnimation ? true : false
  );

  const [phaseInHeader, setPhaseInHeader] = useState(
    noAnimation ? true : false
  );
  const [phaseInDeathMessage, setPhaseInDeathMessage] = useState(
    noAnimation ? true : false
  );
  const [phaseInRevealMessage, setPhaseInRevealMessage] = useState(
    noAnimation ? true : false
  );
  const [phaseInLastWill, setPhaseInLastWill] = useState(
    noAnimation ? true : false
  );
  ...
  return (
    ...
    >
          ...
          content={
            <div className="newspaper-paragraph">
              <ChainedTypewriter
                ...
                chainedFunction={() => {
                  setAnimatedRevealMessage(true);
                  if (!lastWill) onFullyAnimated();
                }}
      ...
      {lastWill && animatedRevealMessage && (
        <PhaseInSection
...
export default function Newspaper(props) {
  ...
  const [animatedTitle, setAnimatedTitle] = useState(
    noAnimation || SKIP_HEADER_ANIMATION ? true : false
  );
  const [animatedSubheader, setAnimatedSubheader] = useState(
    noAnimation || SKIP_HEADER_ANIMATION ? true : false
  );
  const [animatedObituaryCount, setAnimatedObituaryCount] = useState(
    noAnimation ? deaths.length : 0
  );
  const [animatedNoOneDied, setAnimatedNoOneDied] = useState(
    noAnimation ? true : false
  );
  const animationFullyComplete =
    deaths.length > 0
      ? animatedObituaryCount == deaths.length
      : animatedNoOneDied;
  ...
  const [phaseInTitle, setPhaseInTitle] = useState(
    noAnimation || SKIP_HEADER_ANIMATION ? true : false
  );
  const [phaseInSubheader, setPhaseInSubheader] = useState(
    noAnimation || SKIP_HEADER_ANIMATION ? true : false
  );
  const [phaseInNoOneDied, setPhaseInNoOneDied] = useState(
    noAnimation ? true : false
  );
    ...
    if (i > animatedObituaryCount) {
      ...
      return (
        <ObituaryItem
          ...
          death={death}
          onFullyAnimated={() => {
            setAnimatedObituaryCount(animatedObituaryCount + 1);
          }}
          parentProps={props}
        />
      );

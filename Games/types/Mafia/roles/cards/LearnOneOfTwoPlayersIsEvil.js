...
module.exports = class LearnOneOfTwoPlayersIsEvil extends Card {
  constructor(role) {
    super(role);
    ...
    this.actions = [
      {
        ...
        run: function () {
          ...
          var townPlayers = alive.filter((p) => this.game.getRoleAlignment(p.getRoleAppearance().split(" (")[0]) == "Cult" || this.game.getRoleAlignment(p.getRoleAppearance().split(" (")[0]) == "Mafia");

          if(townPlayers.length == 0){
             this.actor.queueAlert(` You learn that no Cult or Mafia are present.`);
          }
          else{

        ...
        if(visits.length == 0){
          ...
          let neighborTarget = Random.randArrayVal(neighbors);
            if(neighborTarget.role.alignment == "Village" && this.actor.role.alignment != "Village"){
              ...
              neighborTarget.setTempAppearance("investigate", Random.randArrayVal(goodPlayers).role.name);
            }
            else{
              neighborTarget.setTempAppearance("investigate", Random.randArrayVal(evilPlayers).role.name);
            } 
        }
          else{
          for (let visit of visits){
            if(visit.role.alignment == "Village" && this.actor.role.alignment != "Village"){
              ...
              visit.setTempAppearance("investigate", Random.randArrayVal(goodPlayers).role.name);
            }
            else{
              visit.setTempAppearance("investigate", Random.randArrayVal(evilPlayers).role.name);
            }
          }
          }

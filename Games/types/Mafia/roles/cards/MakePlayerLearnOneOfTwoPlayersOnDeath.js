  ...
  constructor(role) {
    ...
    this.meetings = {
      "Make Player Learn That One Of Two Players Is Evil On Death": {
        ...
        flags: ["voting"],
        shouldMeet: function () {
          return this.game.graveyardParticipation == "Balling";
        },
        action: {
    ...
    this.listeners = {
      death: function (player, killer, deathType) {
        if (
          player == this.player &&
          (this.data.playerToReveal || this.game.graveyardParticipation)
        ) {
          ...
          let evilPlayers = players.filter(
            (p) => p.role.alignment == "Cult" || p.role.alignment == "Mafia"
          );

          if (this.player.hasEffect("FalseMode")) {
            evilPlayers = players.filter(
              (p) => p.role.alignment != "Cult" && p.role.alignment != "Mafia"
            );
          ...
          victim.queueAlert(
            `A dying ${role} tells you that at least 1 of ${chosenRan[0].name} and ${chosenRan[1].name} is Evil!!!`
          );
        }
      },
    };

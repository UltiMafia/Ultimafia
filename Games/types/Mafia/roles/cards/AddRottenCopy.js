    ...
    this.listeners = {
      addRequiredRole: function (player) {
        ...
        this.player.role.data.hasRottenCopied = true;
          ...
          if(copyCount > shuffledPlayers.length) copyCount = shuffledPlayers.length;
          if(copyCount == 0) return;
            ...
            shuffledPlayers[x].setRole(`${this.player.role.name}:${this.player.role.modifier}`,this.player.role.data, false, true);
            if(x == 0){ 

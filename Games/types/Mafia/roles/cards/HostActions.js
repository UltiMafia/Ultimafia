  ...
  constructor(role) {
    ...
    this.meetings = {
      "Do Host Action": {
        ...
        inputType: "custom",
        targets: ["Nothing", "Grant Poll Vote Immunity", "Run Poll", "Reveal Poll Results", "Gain Gun",],
        action: {
          run: function () {
            ...
            if (this.target === "Run Poll") {
              ...
              for (const player of this.game.alivePlayers()) {
                ...
                let ShareWith = player.holdItem("HostPlayerPoll", this.actor, player, this.actor.role.data.Count);
               this.game.instantMeeting(ShareWith.meetings, [player]);
                }
              }
            }
            else if (this.target === "Grant Poll Vote Immunity") {
                let temp = this.actor.holdItem("HostGrantImmuity", this.actor.role.data.Count);
               ...
               this.game.instantMeeting(gun.meetings, [this.actor]);
                ...
                if(this.actor.role.data.PollVotes == null || this.actor.role.data.PollVotes.length <= 0){
              this.actor.queueAlert(
                ...
                this.game.queueAlert(
                `${maxPlayers.map((p) => p.name)} has received the most Votes with ${max}.`
              );

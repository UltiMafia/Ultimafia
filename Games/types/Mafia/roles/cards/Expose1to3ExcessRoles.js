        ...
        action: {
          ...
          run: function () {
            ...
            if (roles.length <= 0) {
              ...
              var role1 = roles[0];
              if (roles.length == 1) {
                this.actor.queueAlert(`There is only 1 excess role.`);
              }
              this.game.queueAlert(
                `${this.actor.role.name} reveals that 1 of the Excess roles is ${role1}`
              );
              ...
              this.game.queueAlert(
                `${this.actor.role.name} reveals that 2 of the Excess roles are ${role1} and ${role2}`
              );
            } else {
              var role1 = roles[0];
              var role2 = roles[1];

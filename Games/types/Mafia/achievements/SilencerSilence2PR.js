...
module.exports = class SilencerSilence2PR extends Achievements {
  constructor(name, player) {
    ...
    this.listeners = {
      state: function (stateInfo) {
        ...
        if (this.player.role.name == "Silencer" && this.FiddleCount < 2) {
          var action = new Action({
            ...
            run: function () {
              for (let action of this.game.actions[0]) {
                if (
                  action.hasLabels(["effect", "silence"]) &&
      ...
      aboutToFinish: function () {
        if (this.FiddleCount >= 2) {
          this.player.EarnedAchievements.push("Mafia36");
        }
      },
    };
  }

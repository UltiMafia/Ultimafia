...
module.exports = class Blade extends Item {
  constructor(user, target, charge, turn) {
    ...
    if(this.charge == 2){
      ...
      this.targets = ["Typhoon Slash", "Parry", "Focused Fury", "Final Slash"];
      }
    }
      ...
      "Dueling": {
        ...
        targets: this.targets,
         action: {
          ...
          let teammates = this.game.alivePlayers().filter((p) => p.faction == player.faction && p != player);
          let bus;
          if(teammates.length <= 0){
            this.game.sendAlert(`${player.name} uses Bus! ${player.name} had no teammates to Bus.`);
          }
          else{
          bus = Random.randArrayVal(teammates);
          this.game.sendAlert(`${player.name} uses Bus! ${player.name} is Busing ${bus.name}.`);
          ...
          otherPlayer.atk -= 10;
          otherPlayer.def -= 10;
          this.game.sendAlert(`${player.name} uses Fos! ${otherPlayer.name}'s attack power and block power decreased.`);
           player.data.Charge = 0;
          }      
          ...
          this.game.sendAlert(`${player.name} uses Convert! ${otherPlayer.name} is now ${otherPlayer.data.ConversionProgress}% Converted.`);
           player.data.Charge = 0;
          }         
          ...
          this.game.sendAlert(`${player.name} uses Defend! They gain ${block} Block.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          }
          if(player.data.MoveSelected == "Defend+"){
          let block = Random.randInt((player.def+3),(player.def+10))*2
          player.data.Block += block;
          this.game.sendAlert(`${player.name} uses Defend+! They gain ${block} Block.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          player.data.Charge = 0;
          ...
          let block = Random.randInt((player.def+5),(player.def+15))*2;
          player.data.Block += block;
          let info = this.game.createInformation(
            ...
            info.processInfo();
            info.getInfoRaw();
          this.game.sendAlert(`${player.name} uses FMPOV I'm Clear Defense! They gain ${block} Block and their Role is Revealed.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          player.data.Charge = 0;
          }
          if(player.data.MoveSelected == "WIFOM Defense"){
          let block = Random.randInt((player.def+2),(player.def+20))*2;
          player.data.Block += block;
          ...
          this.game.sendAlert(`${player.name} uses WIFOM Defense! They gain ${block} Block and All Village Aligned players are Scrambled.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          player.data.Charge = 0;
          }
          ...
          player.data.Block += block;
          this.game.sendAlert(`${player.name} uses Entrench! They gain ${block} Block and their blocking power is increased.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          player.data.Charge = 0;
          }
          ...
          let block = Random.randInt((player.def),(player.def+7))*2;
          player.data.Block += block;
          this.game.sendAlert(`${player.name} uses Parry! They gain ${block} Block and they are Parrying the next attack.`);
          player.queueAlert(`You now have ${player.data.Block} Block!`);
          player.data.Charge = 0;
          }
          }
          ...
          this.game.sendAlert(`${player.name} uses Towntell Takedown! Their Role is Revealed`);
          ...
          let damage = Random.randInt((player.atk)+5,(player.atk+15));
          let heal = otherPlayer.damage(damage, "basic", player, true, true, true);
          ...
          this.game.sendAlert(`${player.name} uses Final Slash! ${otherPlayer.name}'s Block is removed!`);
          ...
          if(toIncrease == "def"){
          this.game.sendAlert(`${player.name} uses Focus+! Their Blocking power is increased.`);
            player.def += 16;
          }
          if(toIncrease == "crit"){
          this.game.sendAlert(`${player.name} uses Focus+! Their Crit Chance is increased.`);
          ...
          this.game.sendAlert(`${player.name} uses Tunneling Focus! Their Crit Chance is increased and they gain blindness.`);
          player.crit += 4;
          player.giveEffect("Blind", -1);
          player.data.Charge = 0;
          }
          if(player.data.MoveSelected == "Blitz Focus"){
          this.game.sendAlert(`${player.name} uses Blitz Focus! Their Attacking power is increased.`);
          player.atk += 16;
          player.data.Charge = 0;
          }
          if(player.data.MoveSelected == "Worship"){
          this.game.sendAlert(`${player.name} uses Worship! Their Crit Chance, Attacking power, and Blocking power have increased.`);
          player.crit += 1;
          player.atk += 8;
          player.def += 10;
          player.data.Charge = 0;
          ...
          this.game.sendAlert(`${player.name} uses Adrenaline! Their Attacking power has increased.`);
          player.atk += 50;
          }
          ...
          player.atk += 12;
            ...
            this.game.sendAlert(`${player.name} uses Adrenaline! Their Blocking power is increased.`);
            player.def += 18;
          }
          player.data.Charge = 0;
          }
          if(player.data.MoveSelected == "Focused Fury"){
          let stats = ["atk", "atk", "atk", "atk", "def", "crit", "crit", "crit"];
          let toIncrease = Random.randArrayVal(stats);
          if(toIncrease == "atk"){
          this.game.sendAlert(`${player.name} uses Focused Fury! Their Attacking power is increased and their next Move will be Stronger.`);
            player.atk += 13;
          }
          if(toIncrease == "def"){
          this.game.sendAlert(`${player.name} uses Focused Fury! Their Blocking power is increased and their next Move will be Stronger.`);
            player.def += 17;
          }
          if(toIncrease == "crit"){
          this.game.sendAlert(`${player.name} uses Focused Fury! Their Crit Chance is increased and their next Move will be Stronger.`);
            player.crit += 2;
          }
          ...
          if(duelists.indexOf(player) == 0){
            ...
            let blade = player.holdItem("Blade", player, otherPlayer, player.data.Charge, (this.item.Turn+1));
            this.game.instantMeeting(blade.meetings, [player]);
            }
          }
          }
          
        if(this.item.Enemy && this.item.Enemy.alive == false && this.actor.alive == true){
          this.actor.role.data.HasWonBladeDuel = true;
          this.game.sendAlert(`Duel Complete.`);
        }
        else if(this.item.Enemy && this.item.Enemy.alive == true && this.actor.alive == false){

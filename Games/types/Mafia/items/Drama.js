  ...
  constructor(meetingName, dramaQueen, dramaTarget) {
    ...
    this.meetings[meetingName] = {
      ...
      shouldMeet: function (meetingName) {
        ...
        return !!(startDrama?.dramaQueen.alive && startDrama?.dramaTarget.alive);
      },
      action: {
        item: this,
        run: function () {
          ...
          if (this.meeting.votes[dramaTarget.id] === "Reveal the truth") {
            this.game.queueAlert(dramaTarget.name + " decided to reveal the truth and end the drama!");
            dramaTarget.role.revealToAll();
          } else {
            this.game.queueAlert(dramaTarget.name + " will not stand for drama queen's accusations. Their arguing made " + dramaQueen.name + " well known as the drama queen.");

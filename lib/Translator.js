class Translator {
  ...
  numRules() {
    return (
      this.phrases1.length +
      this.phrases2.length +
      this.words1.length +
      this.words2.length +
      this.intraword1.length +
      this.intraword2.length +
      this.prefixes1.length +
      this.prefixes2.length +
      this.suffixes1.length +
      this.suffixes2.length +
      this.regex1.length +
      this.regex2.length +
      this.rev_regex1.length +
      this.rev_regex2.length +
      this.ordering1.length +
  ...
  translate(text, direction) {
    if (direction === "backward" && this.reverseIsDisabled)
  ...
  prefixSwap(prefixes1, prefixes2, text) {
    ...
    for (var i = 0; i < prefixes1.length; i++) {
      text = text.replace(
        new RegExp("\\s" + this.escapeRegex(prefixes1[i]) + "([^\\s])", "g"),
        " " + prefixes2[i] + "$1"
      );
    }
    return text;
  ...
  suffixSwap(suffixes1, suffixes2, text) {
    ...
    for (var i = 0; i < suffixes1.length; i++) {
      text = text.replace(
        new RegExp("([^\\s])" + this.escapeRegex(suffixes1[i]) + "\\s", "g"),
        "$1" + suffixes2[i] + " "
      );
    }
    return text;
  ...
  wordOrdering(ordering1, ordering2, text) {
    for (var i = 0; i < ordering1.length; i++) {
      ...
      var orderString = this.getRelativeOrder(
  ...
  tokenate(s) {
    if (Object.prototype.toString.call(s) === "[object Array]") {
      for (var i = 0; i < s.length; i++) {
        s[i] =
          this.doneToken +
          s[i].toString().split("").join(this.doneToken) +
          this.doneToken;
      }
      return s;
    } else {
      return (
        this.doneToken +
        s.toString().split("").join(this.doneToken) +
        this.doneToken
      );
    }
  }
